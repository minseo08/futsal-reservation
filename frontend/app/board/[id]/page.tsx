...
export default function PostDetailPage() {
  ...
  if (!post) return <div className="p-20 text-center">불러오는 중...</div>;
      ...
      <div className="max-w-3xl mx-auto">
        <article className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-50 mb-8">
          <div className="flex items-center gap-2 mb-4">
            <span className="text-[#4dabf7] font-bold text-sm">{post.category === 'NOTICE' ? '공지사항' : '자유게시판'}</span>
        ...
        <section className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-gray-50">
          <h3 className="text-xl font-bold mb-6 text-[#343a40]">댓글 {post.comments?.length || 0}</h3>
          ...
          {user && (
            <form onSubmit={handleCommentSubmit} className="mb-8">
              ...
              <div className="flex justify-end mt-2">
                <button className="bg-[#4dabf7] text-white px-6 py-2 rounded-xl font-bold hover:bg-[#339af0]">
                  등록
                </button>
              </div>
            </form>
